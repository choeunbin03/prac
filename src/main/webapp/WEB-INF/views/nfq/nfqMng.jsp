...
<body>
	...
	<form id="frm">
		...
		<table>
			...
			<tbody>
				<c:forEach items="${mngList}" var="mngList" varStatus="status">
					<c:set var="nfqType">${mngList.nfqType }</c:set>
					<c:set var="ansYn">${mngList.ansYn }</c:set>
					<tr>
						<td><c:out value="${status.end+1 - status.index}" /></td>
						<td><c:choose>
								<c:when test="${nfqType eq 1 }"	>
									<c:out value="공지" />
								</c:when>
								<c:when test="${nfqType eq 2 }"	>
									<c:out value="FAQ" />
								</c:when>
								<c:otherwise>
									<c:out value="Q&A" />
						...
						<td><a href="#" onclick="fn_goView(${mngList.nfqSeq})">${mngList.nfqTtl}</a></td>
						<td><c:out value="${mngList.qstnCn}" /></td>
						<%-- <td><c:out value="${mngList.ansYn}" /></td> --%>
						<td><c:choose>
								<c:when test="${ansYn eq 'Y' }"	>
									<button type="button" onclick="location.href='/nfq/goUpdateView?nfqType=${nfqType}'">수정/삭제</button>
								</c:when>
								<c:otherwise>
									<button type="button" onclick="location.href='/nfq/goUpdateView?nfqType=${nfqType}'">답변등록</button>
								</c:otherwise>
							</c:choose>
						</td>
						<td><c:out value="${mngList.rgtrDt}" /></td>
						<td><c:out value="${mngList.rgtrId}" /></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
...
<!-- 질문등록하는 부분 구현하기~ -->		
		<button type="button" onclick="location.href='/nfq/regiView?nfqType=1'">공지등록</button>
		<button type="button" onclick="location.href='/nfq/regiView?nfqType=2'">FAQ등록</button>
